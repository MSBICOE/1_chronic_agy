...
cd_data <- read_csv("data/chronic_disease_with_variables_agegroup_1.csv")
...
cd_data.1 <- cd_data %>% 
  mutate(age_group_sort = case_when(age_group == "All Age" ~ 1,
                                    ...
                                    age_group == "> 60" ~ 4,
                                    age_group == "Within 2 years" ~ 5,
                                    age_group == "Within 5 years" ~ 6,
                                    TRUE ~ 7)) %>% 
  group_by(gender, age_group, data_use) %>% 
  mutate(link_overall = n(),
         ProbNorm_overall = round(1 / (25 * 24) * 100, 3),
         cases_overall = sum(instance),
         MU_overall =  mean(instance),
         SD_overall = sd(instance),
         prob_overall = instance / cases_overall * 100,
         z_overall = (instance - MU_overall) / SD_overall,  ## (SD_overall / sqrt(link_overall))
  ...
  mutate(link_from = n(),
         ...
         z_from = (instance - MU_from) / SD_from,
  ...
  mutate(prob = round(prob, 2),
         ...
         d_type_order = ifelse(develop_type == "overall", 1, 2),
         ...
         link_width = case_when(z < 1.645 ~ 1,
                             ...
                             z < 2.58 ~ 3,
                             TRUE ~ 4),
         link_color = case_when(link_width == 1 ~ c4[1],
                                link_width == 2 ~ c4[2],
                                link_width == 3 ~ c4[3],
                                TRUE ~ c4[4]
                                )
