...
cd_data <- read_csv("data/chronic_disease_with_variables_agegroup_new_w.csv") %>% 
  mutate(age_group_2 = factor(age_group,
                            levels = c( "All Age", "< 40", "40 to 60", "> 60"),
                            labels = c( "All Age", "A1", "A2", "A3")),
         develop_year_2 = factor(develop_year,
                            levels = c( "All Years", "Within 2 years",  "Within 5 years",  "Within 10 years"),
                            labels = c( "All Years", "Y1",  "Y2",  "Y3")))
...
var_df_all <- expand.grid(cd = cd_df$cd_from_code, develop_type = d_types) %>% 
...
for (dt in d_types) {
  ...
  for (du in d_uses) {
    ...
    for (i in seq_along(1:len)) {
      ...
      head_2 <- paste("Difference on ", d_use, " by Gender")
